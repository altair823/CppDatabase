...
TEST(RecordTest, VectoredTest){
  ...
  auto schema = schema_builder.set_field(TypeKind::DATETIME, "Created date").unwrap()
      ...
      ->set_field(TypeKind::STRING, "File name", KeyType::PK).unwrap()
      ->build().unwrap();
  std::vector<RecordShared> records;
  for (int i = 0; i < 3; i++){
    records.push_back(RecordFactory::create(schema));
  }
  for (auto& s: records){
...
//    std::cout<<s<<std::endl;
//  }
  RecordShared exp_record = RecordFactory::create(schema);
  ...
  //std::cout<<*records[0]<<std::endl<<*exp_record<<std::endl;
...
TEST(RecordTest, DeserializeTest){
  ...
  RecordShared deserialized_record = RecordFactory::create(schema);
